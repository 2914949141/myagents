...
def _run_single_subagent(agent_type: str, task: str) -> str:
    ...
    result, _ = runner.run(sub_history, sub_tools, max_turns=spec['max_turns'])
    return result

# 当LLM一次发出多个dispatch_subagent时，在agent_runner李并行执行
def _execute_tools_parallel(tool_calls: list, tracer=None) -> list:
    """并行执行多个工具调用"""
    results = []
    ...
    for tc in other_calls:
        output = execute_basic_tool(tc, tracer=tracer)
...
class DispatchSubagentTool(BaseTool):
    ...
    def execute(self, tool_call) -> str:
        ...
        final_result, _ = runner.run(sub_history, sub_tools, max_turns=spec['max_turns'])
