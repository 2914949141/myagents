...
from agent_hemo.settings import COMMAND_TIMEOUT, COMMAND_AUTO_APPROVE
from agent_hemo.tools.base_tool import BaseTool
from agent_hemo.utils.sandbox import confirm_dangerous_action
...
class CommandTool(BaseTool):
    ...
    def execute(self, tool_call) -> str:
        ...
        if not COMMAND_AUTO_APPROVE and not confirm_dangerous_action("执行命令", command):
            return "用户拒绝了命令执行"
        ...
        try:
            result = subprocess.run(
                command, 
                shell=True, 
                cwd=str(PROJECT_ROOT),   # 关键：命令总在项目根目录下执行
                capture_output=True, 
                text=True,
