...
_current_tracer: ContextVar["AgentTracer | None"] = ContextVar("current_tracer", default=None)

from agent_hemo.settings import LOGS_DIR, UTC8

LOGS_DIR.mkdir(exist_ok=True)

def set_current_tracer(tracer: "AgentTracer | None"):
    return _current_tracer.set(tracer)
def reset_current_tracer(token):
    if token is None:
        return 
    try :
        return _current_tracer.reset(token)
    except ValueError:
        # SSE 生成器跨 yield 后 Context 可能不一致，忽略即可
        pass
    
def get_current_tracer() -> "AgentTracer | None":
